# -*- coding: utf-8 -*-
"""69 - 방향=더캔지추 우위표(68 bias) + 타점=10분봉 기본더블비(추세 눌림). 2026년만(빠른 체크).
방향: 1H 7요소 가중 bias(68과 동일). 타점: 10m에서 bias방향으로 4/4밴드 터치 시 봉마감 진입(단일포지션).
  bias LONG -> 10m 저가<=하단BB(4,4) 매수 / bias SHORT -> 고가>=상단BB(4,4) 매도.
청산: 1분봉 시간순, TP=+1.5*ATR10 / SL=-1.5*ATR10 (ATR10=10m ATR14, KTR 대용). 비용$0.30, KST08~24, 2%복리.
비교: 무게이트(모든 더블비터치) / 일치(터치방향==bias) / 반대(통제). 워밍업 2025-10~, 진입 2026만.
data/에서 실행. 콘솔 ASCII만(규칙3)."""
import csv, math, time, bisect

WARM_CUT=1759276800   # 2025-10-01 UTC (워밍업 시작)
SPREAD=0.30; RISK=0.02; START_H=8; CAP1M=20000   # 단일포지션이라 캡 짧게(1m봉수)
TPx=1.5; SLx=1.5
...
for i in range(m):
    ...
    if kyear(MT[i])!="2026" or khour(MT[i])<START_H: continue
    ...
    if slope>0 and ML[i]<=b2l[i] and mma20[i]>e:        # 상승추세 + 하단 눌림 = 매수
        ...
        cand.append((ee,e,-1,matr[i],bdir,bs))
print(f"기본더블비 후보(2026, 10m 20MA 방향+4/4터치, KST08~): {len(cand)}건\n")

def run(filt):
    eq=[1.0];cum=0.0;wins=0;busy=-1;nn=0
    ...
    def mdd(E):
        ...
        return 100*mx
    return nn,(100*wins/nn if nn else 0),cum,100*(eq[-1]-1),mdd(eq)

print(f"# 10m 기본더블비 + KTR그리드(base=ATR10) | TP=20MA / 그리드0~-4.5 랏1·1·2·2·3·4 / 풀스톱-5 | 2026 | 비용${SPREAD}/2%/KST08~24\n")
print(f"{'필터':>18}{'진입':>6}{'승률':>7}{'누적R':>8}{'수익%':>8}{'MDD':>7}")
...
for lab,f in defs:
    nn,wr,cum,ret,md=run(f)
    print(f"{lab:>18}{nn:>6}{wr:>6.1f}%{cum:>+8.1f}{ret:>+7.1f}%{md:>6.1f}%")
