# -*- coding: utf-8 -*-
"""Slice diagnostics for immediate sweep-reversal RR2 best-quality branch."""
...
INPUT = ROOT / "result" / "session_sweep_reversal_immediate_rr2_full" / "best_quality_trades.csv"
OUTPUT_DIR = ROOT / "result" / "sweep_reversal_immediate_slice_diagnostics"
TRADING_DAYS = 1075
TRADING_DAYS_2026 = 142


spec = importlib.util.spec_from_file_location("base100_for_121", BASE_SCRIPT)
base = importlib.util.module_from_spec(spec)
sys.modules["base100_for_121"] = base
...
def candidate_masks(df: pd.DataFrame) -> list[tuple[str, pd.Series]]:
    ...
    for col in ["year", "session", "level_name", "direction", "hour", "weekday", "risk_bucket", "distance_r_bucket", "adr60_bucket", "vol20_bucket", "ret60_bucket"]:
    ...
    # Focused combinations that remain implementable.
    simple_cols = ["session", "level_name", "direction", "risk_bucket", "adr60_bucket", "ret60_bucket"]
    atoms: list[tuple[str, pd.Series]] = []
    for col in simple_cols:
        for value in sorted(df[col].dropna().unique(), key=lambda x: str(x)):
            mask = df[col].astype(str) == str(value)
            if 100 <= int(mask.sum()) <= 3500:
...
def write_html(results: pd.DataFrame) -> None:
    ...
    header{background:#243b53;color:#fff;padding:30px 42px}h1{font-size:26px;margin:0 0 8px}p{margin:0;color:#d9e2ec}
    ...
    html = """<!doctype html><html lang='ko'><head><meta charset='utf-8'><title>Immediate Sweep Reversal Slice Diagnostics</title><style>%s</style></head>
<body><header><h1>Immediate Sweep Reversal Slice Diagnostics</h1><p>Pre-entry slices for the best full-period immediate sweep-reversal RR2 branch.</p></header><main>
%s%s%s
</main></body></html>""" % (
        css,
    ...
    (OUTPUT_DIR / "sweep_reversal_immediate_slice_diagnostics_report.html").write_text(html, encoding="utf-8")
...
def run() -> None:
    ...
    results.to_csv(OUTPUT_DIR / "sweep_reversal_immediate_slice_diagnostics.csv", index=False, encoding="utf-8-sig")
    write_html(results)
    print("=== SWEEP REVERSAL IMMEDIATE SLICE DIAGNOSTICS ===")
