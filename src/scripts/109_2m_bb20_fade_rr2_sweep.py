# -*- coding: utf-8 -*-
"""2m BB20 fade fixed 1:2 RR sweep.

Idea:
- Mean reversion, not the prior BB20 wick -> BB4 pullback concept.
- Short when price closes or wicks above the BB20/2 upper close band.
- Long when price closes or wicks below the BB20/2 lower close band.
- Entry on the next 2m open.
- Stop beyond the signal candle extreme plus a buffer.
- Target is exactly 2R from entry.

The goal is to test whether a simpler high-frequency fixed RR2 concept can
reach 10-20 trades/day while staying profitable beyond the 2026 sample.
"""
...
OUTPUT_DIR = ROOT / "result" / "bb20_fade_rr2_sweep"


spec = importlib.util.spec_from_file_location("base100_for_109", BASE_SCRIPT)
base = importlib.util.module_from_spec(spec)
sys.modules["base100_for_109"] = base
...
STOP_BUFFERS = env_float_list("STOP_BUFFERS", [0.2, 0.5, 1.0])
MIN_RISKS = env_float_list("MIN_RISKS", [0.5, 0.8, 1.2])
MAX_RISKS = env_float_list("MAX_RISKS", [3.0, 5.0, 8.0])
MAX_HOLD_BARS_SET = env_int_list("MAX_HOLD_BARS_SET", [10, 20, 40])
...
def find_entries(df: pd.DataFrame, signal_mode: str, cooldown_bars: int) -> pd.DataFrame:
    idx = df.index
    ...
    mid = df["bb20_2_mid_close"].to_numpy(float)
    ...
    for signal_pos in range(120, len(df) - 2):
        ...
        if signal_mode == "close":
            short_signal = close[signal_pos] > upper[signal_pos]
            long_signal = close[signal_pos] < lower[signal_pos]
        elif signal_mode == "wick":
            short_signal = high[signal_pos] > upper[signal_pos]
            long_signal = low[signal_pos] < lower[signal_pos]
        else:
        ...
        rows.append({
            ...
            "signal_mid": float(mid[signal_pos]),
            "band_distance": float(upper[signal_pos] - lower[signal_pos]),
...
def write_html(summary: pd.DataFrame) -> None:
    ...
    html = """<!doctype html><html lang='ko'><head><meta charset='utf-8'><title>2m BB20 Fade RR2 Sweep</title><style>%s</style></head>
<body><header><h1>2m BB20 Fade RR2 Sweep</h1><p>Fixed 1:2 RR mean-reversion entries after BB20/2 extremes.</p></header><main>
%s%s
</main></body></html>""" % (
        css,
    ...
    (OUTPUT_DIR / "bb20_fade_rr2_sweep_report.html").write_text(html, encoding="utf-8")
...
def run():
    ...
    for signal_mode in [x.strip() for x in SIGNAL_MODES if x.strip()]:
        for cooldown_bars in COOLDOWN_BARS_SET:
            entries = find_entries(df, signal_mode, cooldown_bars)
            print("ENTRIES", signal_mode, "cooldown", cooldown_bars, len(entries), flush=True)
                    ...
                    for max_risk in MAX_RISKS:
                        ...
                        for max_hold_bars in MAX_HOLD_BARS_SET:
                            for cap in CONCURRENCY_CAPS:
                                ...
                                config_id = "mode%s_cd%s_sb%s_min%s_max%s_hold%s_cap%s" % (
                                    signal_mode,
                                    cooldown_bars,
    ...
    summary.to_csv(OUTPUT_DIR / "bb20_fade_rr2_sweep_summary.csv", index=False, encoding="utf-8-sig")
    if best_trades is not None:
        best_trades.to_csv(OUTPUT_DIR / "bb20_fade_rr2_best_trades.csv", index=False, encoding="utf-8-sig")
    write_html(summary)

    print("=== 2M BB20 FADE RR2 SWEEP ===")
